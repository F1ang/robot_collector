...
extern int16_t robot_target_speed[3];  //机器人目标速度 X Y Yaw
...
void move_task(void const * argument)
{
	while(1)
	{
		...
		//灰度传感器( 速度x,y,yaw属于(0~1000) )
		/* 
			robot_target_speed[0] = 5			
		*/
		control_flag.turn_yaw = robot_target_speed[0];

		if (control_flag.turn_yaw == 0) 		//停车
		{
			Stop_Line(robot_target_speed);
		}
		else if (control_flag.turn_yaw == 5) 	//基础巡线
		{
			Find_Line(robot_target_speed);
		}
		else if (control_flag.turn_yaw == 2)	//直行路口
		{
			Goforword(robot_target_speed);
		}	
		else if (control_flag.turn_yaw == 1)	//左拐路口
		{
			Turn_Left(robot_target_speed);
		}	
		else if (control_flag.turn_yaw == 1)	//右拐路口
		{
			Turn_Right(robot_target_speed);
		}
		...
		//printf("%d %d\r\n",encoder_delta_target[0], encoder_delta[0]);
		//电机PID速度控制
		...
		//电压死区 pwm30	
		if(labs(motor_pwm[0])<30)motor_pwm[0] = 0; 
		if(labs(motor_pwm[1])<30)motor_pwm[1] = 0; 
		if(labs(motor_pwm[2])<30)motor_pwm[2] = 0; 
		if(labs(motor_pwm[3])<30)motor_pwm[3] = 0; 

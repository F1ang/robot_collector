...
int16_t PID_Motor_Control(int8_t Motor_Num, int16_t speed_target, int16_t speed_current)
{
	...
	//限制最大输出
//	if(motor_pwm_out[Motor_Num-1] > 1)
//		motor_pwm_out[Motor_Num-1] = 2000;
//	if(motor_pwm_out[Motor_Num-1] < -2000)
//		motor_pwm_out[Motor_Num-1] = -2000;
	if(motor_pwm_out[Motor_Num-1] > 1000)
		motor_pwm_out[Motor_Num-1] = 1000;
	if(motor_pwm_out[Motor_Num-1] < -1000)
		motor_pwm_out[Motor_Num-1] = -1000;
...
int16_t PID_FindLine(int16_t speed_target, int16_t speed_current)
{
	...
	//PID计算电机输出PWM值
	motor_pwm_out += line_kp*bias*PID_SCALE +line_kd*(bias-bias_last)*PID_SCALE + line_ki*bias_integral*PID_SCALE;
	
	//记录上次偏差
	bias_last = bias;
	
	//限制最大输出
	if(motor_pwm_out> 1000)
		motor_pwm_out = 1000;
	if(motor_pwm_out < -1000)
		motor_pwm_out = -1000;

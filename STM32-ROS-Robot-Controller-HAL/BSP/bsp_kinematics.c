...
double pulse_per_meter = ENCODER_RESOLUTION/(WHEEL_DIAMETER*3.1415926); //每米脉冲:一圈总脉冲/(2*pi*r)，即x/m每米多少个脉冲1320/0.2041=6467.4
double linear_correction_factor = 1.0;          //线性校正系数
double angular_correction_factor = 1.0;			//角校正系数
...
void Kinematics_Init(int16_t* robot_params)
{
	...
	//轮子转动一圈，移动的距离为轮子的周长WHEEL_DIAMETER(2r)*3.1415926
...
//	v_w[3] = -v_tx + v_ty + (rx_plus_ry_cali)*omega;
	...
	output[0] = (int16_t)(v_w[0] * pulse_per_meter/PID_RATE);//->m/s * x/m  /time  
...
//	static double delta_v_integral[2];
	...
	//printf("%f %f %f %f\r\n",dv_w_times_dt[0],dv_w_times_dt[1],dv_w_times_dt[2],dv_w_times_dt[3]);

...
static const u16 circle_limit_table[82] =
    {
        ...
        21051, 20932, 20815, 20757, 20643, 20587, 20476, 20421, 20312, 20205, 20152, 20048, 19996, 19894, 19844, 19744, 19645};
#endif
...
void RevPark_Circle_Limitation(void)
{
    ...
    if (temp > (u32)((MAX_MODULE * MAX_MODULE))) // (Vd^2+Vq^2) > MAX_MODULE^2
    {
        ...
        // 2*32768^2分128等分=>2*32768^2/128=32768*512
        // temp=MAX_MODULE^2在等分的位置=>temp=MAX_MODULE^2/(32768*512)  注:MAX_MODULE(此值随着PID输出变化)
        // 因过调制只有temp过大才进行,有最小的index_min=MAX_MODULE^2/(32768*512) 注:MAX_MODULE(此值仅确定最小值偏移值,按调制比固定,比如32111)
        // 128-index_min=可利用的index
        // circle_limit_table=MMI*R1, R1=MAX_MODULE->偏差越小,拉回作用越小,调制比越大.   MMI*R1/32768,
        temp /= (u32)(512 * 32768);           //  (1)过调制量temp = (Vq^2+Vd^2)/(32768*512)
        temp -= START_INDEX;                  //  (2)最小过调制量START_INDEX=MAX_MODULE^2/(32768*512)
        index = circle_limit_table[(u8)temp]; //  (3)偏移量index=temp - START_INDEX -> 查表

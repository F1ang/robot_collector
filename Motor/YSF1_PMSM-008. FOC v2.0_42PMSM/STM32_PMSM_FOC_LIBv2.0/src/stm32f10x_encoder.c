...
#if ((defined(TIMER2_HANDLES_ENCODER) && defined(TIMER3_HANDLES_ENCODER)) || (defined(TIMER2_HANDLES_ENCODER) && defined(TIMER4_HANDLES_ENCODER)) || (defined(TIMER3_HANDLES_ENCODER) && defined(TIMER4_HANDLES_ENCODER)))
...
s16 ENC_Calc_Rot_Speed(void)
{
    ...
    if (!bIs_First_Measurement) {
        ...
        if (hEnc_Timer_Overflow_sample_one != hEnc_Timer_Overflow_sample_two) { // Compare sample 1 & 2 and check if an overflow has been generated right
        ...
        } else { // encoder timer up-counting
            wDelta_angle = (s32)(hCurrent_angle_sample_one - hPrevious_angle +
                                 (hEnc_Timer_Overflow_sample_one) * (4 * ENCODER_PPR));
        }

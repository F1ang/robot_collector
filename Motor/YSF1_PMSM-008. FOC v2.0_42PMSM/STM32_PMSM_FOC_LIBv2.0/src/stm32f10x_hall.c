...
s16 HALL_GetRotorFreq(void)
{
    ...
    if (DoRollingAverage) {
        PeriodMeasAux = GetAvrgHallPeriod(); // 滑动均值滤波PSC、CAP、DIR
    } else {                                 // Raw period
                ...
                if (PeriodMeasAux.wPeriod > MAX_PERIOD) /* Speed is too low */
                {
                    ...
                    if (PeriodMeasAux.wPeriod > (u32)SPEED_OVERFLOW) {
                        if (HALL_GetCaptCounter() < 2) // First capture must be discarded
                        {
                            ...
                            // f_hall=clk/period    f_foc=2*f_pwm/(REP_RATE+1)   2pi~65536~3HALL状态划分
                            // PWM周期的频率下,dpp=f_hall/f_foc * 65536/3
...
void HALL_IncElectricalAngle(void)
{
    ...
    if (hRotorFreq_dpp != HALL_MAX_PSEUDO_SPEED) {
        hElectrical_Angle += hRotorFreq_dpp; // 1hz的电角度
...
void HALL_Init_Electrical_Angle(void) // 预定位
{
    ...
    case STATE_5:
        // S16_PHASE_SHIFT为同步电角度值(计算当前HALL上升沿对应的电角度shift+offset=360°,滞后的话)
        hElectrical_Angle = (s16)(S16_PHASE_SHIFT + S16_60_PHASE_SHIFT / 2); // A相反电动势最高点定位电角度0°,同步电角度(HALLA的上升沿与0°的滞后/超前)
        break;
    case STATE_1:
...
#if (HALL_SENSORS_PLACEMENT == DEGREES_120)
        switch (bHallState) { // 3状态(5 3 6),按120°
        case STATE_5:
            if (bPrevHallState == STATE_5) {
                // a speed reversal occured
                if (bSpeed < 0) {
                    bSpeed = POSITIVE_SWAP; // 转到半逆转
                } else {
                    bSpeed = NEGATIVE_SWAP;
                }
            } else if (bPrevHallState == STATE_6) { // 逆正
                bSpeed = POSITIVE;

...
#define HC4_INIT (s16)(((wK2_LO * MAX_CURRENT) / MAX_BEMF_VOLTAGE) * F2 / SAMPLING_FREQ)
// Do not be modified
#define SLESS_T_ALIGNMENT_PWM_STEPS (u32)((SLESS_T_ALIGNMENT * SAMPLING_FREQ) / 1000)
#define SLESS_ALIGNMENT_ANGLE_S16   (s16)((s32)(SLESS_ALIGNMENT_ANGLE) * 65536 / 360)
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
...
void STO_StateObserverInterface_Init(void)
{
    ...
    StateObserver_ConstStruct.hC4 = (s16)((((wK2_LO * MAX_CURRENT) / (MAX_BEMF_VOLTAGE)) * F2) / (SAMPLING_FREQ));

...
*******************************************************************************/
/*******************************************************************************
* 描    述: 函数用于配置微处理器外设来读取三相电阻拓扑电流和产生 PWM。
*           函数初始化 NVIC、ADC、GPIO、TIM1 外设。
*           特别，ADC 和 TIM1 外设配置成每 PWM 转换周期同时运行两路 A/D 转换。
* 提    示: 必须在主函数中调用。
*******************************************************************************/
...
*******************************************************************************/
/*******************************************************************************
* 描    述: 函数功能用于存储对应零电流的三路模拟电压，用于补偿运放产生的零漂。
* 提    示: 函数读取用于电流采集的 ADC 通道模拟电压值。因此必须在 PWM 输出前
*           调用， 以便通过逆变器电流为零。 这些值存储在 Phase_x_Offset 变量中。
*******************************************************************************/
...
*******************************************************************************/
/*******************************************************************************
* 描    述: 函数从A/D转换外设获得的电流值开始计算q1.15格式中A相和B相电流值。
* 提    示: 为了取得q1.15格式的电流值，当读取相电流A/D转换值时，必须减去与漂移
*           对应的数字量。因此，函数必须在SVPWM_3ShuntCurrentReadingCalibration
*           之后调用。
*******************************************************************************/
...
*******************************************************************************/
/*******************************************************************************
* 描    述: 在FOC算法运算之后，计算定子新电压Vα和Vβ。函数功能是从这些电压值开
*           始的逆变器支路准确计算三个周期。并且，一旦下一次PWM周期三个周期已知，
*           函数设置通道4输出与用于设置下一次电流读取的采样点的寄存器对比。特殊
*           情况下，依据周期值，计算采样点。
*******************************************************************************/
...
*******************************************************************************/
/*******************************************************************************
* 描    述: 用于超前电流读取的启动和关闭。如果禁止电流超前读取则电流读取与事件
*           更新同时执行。
*******************************************************************************/
...
*******************************************************************************/
/*******************************************************************************
* 描    述: 在ISR转换完成之内执行程序。计算总线电压和温度传感器采集和关闭ADC外
*           部触发使能。
*******************************************************************************/

...
/*
  ...
  指针函数声明->结构体继承->结构体数组初始化->回调 (ps:声明时可直接初始化)
  1、elf文件->linux下的可执行文件,还可以背链接成bin文件,包含机器类型、程序分段(代码段、数据段、符号表)->pytest加载符号表(对应map表)
  2、gdb->可调式的进程(启动、终止、添加、暂停等)
  3、public、private、protected
  变量:public-对象均可访问,private-仅该类成员函数可访问(类内),protected-子类+该类可访问(类内)
  方法:均可访问
  类继承方式:public-派生类保持基类属性  private-派生类使基类全为private(类内),导致派生类外部不可见  protected-基类的public+protected变protected(类内访问)
  4、list:[],随时增删改
  Tuple: (),无法增删改
  tict: {},键值对,键唯一,值更改 “:”分隔

  try .. except ERROR (抛出异常)

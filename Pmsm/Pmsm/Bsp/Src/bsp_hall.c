...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
        ...
        foc_data_handler.hall_ops.hall_cap = (foc_data_handler.hall_ops.overcount * 0x10000L) + HAL_TIM_ReadCapturedValue(&htim3, TIM_CHANNEL_1); // 需考虑溢出
        ...
        if (foc_data_handler.m_mode == MOTOR_RUN) {
            ...
            foc_data_handler.hall_ops.dpp_sum -= foc_data_handler.hall_ops.buffer[foc_data_handler.hall_ops.buffer_index];
            ...
            // 滑动均值滤波
            foc_data_handler.hall_ops.buffer[foc_data_handler.hall_ops.buffer_index] = foc_data_handler.hall_ops.dpp;
            foc_data_handler.hall_ops.dpp_sum += foc_data_handler.hall_ops.buffer[foc_data_handler.hall_ops.buffer_index];
            ...
            foc_data_handler.hall_ops.avg_dpp = (foc_data_handler.hall_ops.dpp_sum / WBuffer_MAX_SIZE);
        }
    }
}
...
float hall_positionEst(foc_handler *foc_data)
{
    ...
    /***********************电角度从0到2PI往复 **************************/
    foc_data->hall_ops.hall_theta_inter = (float32_t)(foc_data->hall_ops.hall_theta * theta_k); //<theta_inter插值法估算的电角度，就是文中的θ_use浮点形式
...
float hall_pll_filter(foc_handler *foc_data)
{
    ...
    error = foc_data->hall_ops.hall_theta * theta_pll_k - foc_data->hall_ops.pll_theta;
    ...
    /*************************锁相环法电角速度的限幅 ****************************/
    foc_data->hall_ops.pll_theta += (foc_data->hall_ops.pll_omega + foc_data->hall_ops.pll_kp * error) * foc_data->hall_ops.pll_Te;
    ...
    foc_data->hall_ops.pll_ki = 0.236f * foc_data->hall_ops.omega_inter * foc_data->hall_ops.omega_inter; // 0.236f约等于根号5-2
...
void foc_processHF_IT(foc_handler *foc_data, ADC_HandleTypeDef *hadc)
{
    ...
    // static float speedP = (float)(1.0f / P);                  //<乘积因子，电角速度=极对数*机械角速度

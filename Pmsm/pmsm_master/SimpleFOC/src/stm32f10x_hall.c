...
static s16 hElectrical_Angle;     // 电角度
static s16 hRotorFreq_dpp;        // 转子电频率dpp
...
* 	    best results.
*******************************************************************************/
s16 HALL_GetSpeed ( void )  // 角速度0.1Hz
{ 
  s32 wAux;
...
* 	    best results.
*******************************************************************************/
s16 HALL_GetRotorFreq ( void )  // 转子电频率
{
...
*******************************************************************************/
/**
 * @description: TI1->捕获脉冲计数CC1->捕获中断
 * @param {TI1->捕获脉冲计数CC1->捕获中断
...
// Check for the source of TIMx int - Capture or Update Event - 
  if ( TIM_GetFlagStatus(HALL_TIMER, TIM_FLAG_Update) == RESET )
  {
    ...
    bHallState = ReadHallState();               // 获取HALL位置
    switch(bHallState)
    {
      case STATE_5:
  ...
  //Update Rotor Frequency Computation
   hRotorFreq_dpp = HALL_GetRotorFreq();   // 转子电频率dpp
